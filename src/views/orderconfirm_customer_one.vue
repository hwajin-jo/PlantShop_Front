    ...
    <div id="body">
        ...
        <table style="border: 1px solid black; text-align: center; margin-bottom: 80px;">
          ...
          /></td>
                    ...
                    <td style="border: 1px solid black;">{{currentCart.product.pprice.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')}} 원</td>
                    <td style="border: 1px solid black;">{{currentCart.pquantity}}</td>
                    <td style="border: 1px solid black;">{{(currentCart.product.pprice * currentCart.pquantity).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')}} 원</td>
                </tr>
            </tbody>
        </table>
...
<script>
    ...
    export default{
        ...
        methods: {
            ...
            getCartList(cid) {
                CartService.getbyId(cid)
                .then((response) => {
                    this.currentCart = response.data;
                    console.log(response.data);
                    console.log(response.data.length);
                    var total = this.currentCart.product.pprice * this.currentCart.pquantity;
                    var totalQty = this.currentCart.pquantity;
                    this.total = total;                    
      ...
      new window.daum.Postcode({
        ...
        mounted(){
            this.getaddress();
            this.getCartList(this.$route.params.cid);
        }
    }
</script>
